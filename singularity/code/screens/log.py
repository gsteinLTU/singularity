...
class LogScreen(dialog.ChoiceDialog):
    def __init__(self, parent, pos=(0.5, 0.5), size=(0.73, 0.63), *args, **kwargs):
        ...
        self.filter_log = button.FunctionButton(
            self,
            (-1.0, 0.0),
            (-0.18, -0.08),
            autotranslate=True,
    ...
    def rebuild(self):
        self.key_list = [
            message for message in g.pl.log if not type(message) in filtered_log_class
        ]
    ...
    def render_log_message(self, message):
        ...
        return "%s -- %s" % (
            _("DAY") + " %04d, %02d:%02d:%02d" % log_emit_time,
            log_message,
        )
...
class FilterLogDialog(dialog.MessageDialog):
    ...
    def rebuild(self):
        for log_type, log_class in logmessage.SAVEABLE_LOG_MESSAGES.items():
            ...
            self.log_class_toggles[log_type].active = (
                log_class not in filtered_log_class
            )
